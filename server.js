...
async function optimizeVideoWithFFmpeg(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // - -crf 25: хороший баланс качества и размера (для 720p)
    ...
    // - -b:v 500k: целевой битрейт 500 kbps (примерно 3.75 МБ/мин)
    const ffmpegArgs = [
      '-i', inputPath,
      '-c:v', 'libx264',
      '-preset', 'veryfast',
      '-profile:v', 'main',
      '-crf', '25', // Баланс качества и размера для 720p
      '-g', '30',
      '-keyint_min', '30',
      '-sc_threshold', '0',
      '-r', '30',
      '-b:v', '500k', // Целевой битрейт 500 kbps (~3.75 МБ/мин)
      '-maxrate', '750k', // Максимальный битрейт
      '-bufsize', '1000k', // Размер буфера
